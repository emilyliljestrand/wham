...
plot.sel.blocks <- function(mod, ages, ages.lab, plot.colors, indices = FALSE, do.tex = FALSE, do.png = FALSE, fontfam="", res = 72, use.i, od)
{
  ...
  if(inherits(mod$sdrep, "sdreport")) {
    sel.se.all <- lapply(TMB:::as.list.sdreport(mod$sdrep, what = "Std", report = TRUE)$selAA,
  ...
  rfns <- gsub(pattern, "_", fleet_region_names)
    ...
    if(!is.null(sel.se.all)) {
      sel.se <- do.call(rbind, sel.se.all)[blocks,,drop=FALSE]
      ...
      if(!is.null(sel.se)) polygon(c(ages, rev(ages)), c(sel.low[j,], rev(sel.high[j,])),
