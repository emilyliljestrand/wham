...
for(s in 1:dat$n_stocks){
  out = readRDS(paste0(stock.names.f[s], "_SSB_table.RDS"))
  out = round(out, 0)
  out <- out |> knitr::kable(format = type, booktabs = T, escape=F, longtable = T, row.names = T,
    label = paste0(stock.names.l[s], "-SSB-table"),
      caption=paste0("Total spawning stock biomass for ", stock.names.tab[s], ".")) |>
      kableExtra::kable_styling(latex_options=c("basic", "repeat_header"))
  cat(knitr::knit_print(out))
}
```
